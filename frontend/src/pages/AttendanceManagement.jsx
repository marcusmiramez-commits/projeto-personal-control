            ...
            <table className="w-full border-collapse min-w-max" data-testid="attendance-table">
              ...
              <tbody>
                {attendanceRows.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    <td className="bg-gradient-to-br from-blue-600 to-blue-700 text-white p-0.5 md:p-2 border border-blue-500 sticky left-0 z-10">
                      {row.studentId ? (
                        <div className="font-semibold px-0.5 md:px-2 text-[9px] md:text-base leading-tight truncate">{row.studentName}</div>
                      ) : (
                        <Select onValueChange={(value) => handleStudentNameChange(rowIndex, value)}>
                          <SelectTrigger className="bg-white text-slate-900 border-0 h-6 md:h-8 text-[9px] md:text-base px-1">
                    ...
                    <td className="bg-slate-50 p-0.5 md:p-2 border border-slate-200 text-center">
                      <button
                        onClick={() => removeRow(rowIndex)}
                        className="p-0.5 md:p-1 hover:bg-red-100 rounded"
                        data-testid={`remove-row-${rowIndex}`}
                      >
                        <Trash2 className="w-2.5 h-2.5 md:w-4 md:h-4 text-red-600" />
                      </button>
                    </td>
                  </tr>
                ))}
